...
public class POMDP {	
	...
	public int nextStateRDM(int currentState, int selectedaction) {
		...
		//sRDMSimConnector.timedRDMSim.network.setEffector(RDMSimConnector.effector);
		System.out.println("----------------------------"+RDMSimConnector.timestep);
		TopologyStrategy current = null;
		if(selectedaction==0)
		{
			// RDMSimConnector.timedRDMSim.network.setTopologyStrategy(new BalancedTreeTopologyStrategy(), RDMSimConnector.timestep);
			System.out.println("BT");
			ResultsLog.lat = RDMSimConnector.effector.setStrategy(new BalancedTreeTopologyStrategy(), RDMSimConnector.timestep).getEffect().getLatency();
			current=new BalancedTreeTopologyStrategy();

		}
		else if(selectedaction == 1)
		{
			// RDMSimConnector.timedRDMSim.network.setTopologyStrategy(new FullyConnectedTopology(), RDMSimConnector.timestep);

			System.out.println("FC");
			ResultsLog.lat = RDMSimConnector.effector.setStrategy(new FullyConnectedTopology(), RDMSimConnector.timestep).getEffect().getLatency();
			current=new FullyConnectedTopology();
		...
		else if(selectedaction == 2)
		{
			...
			ResultsLog.lat = RDMSimConnector.effector.setStrategy(new NConnectedTopology(), RDMSimConnector.timestep).getEffect().getLatency();
			current=new NConnectedTopology();

		}

		int al = RDMSimConnector.timedRDMSim.network.getNumActiveLinks();
		int tl= RDMSimConnector.timedRDMSim.network.getNumTargetLinks();
		double bw =  RDMSimConnector.timedRDMSim.network.getBandwidthUsed(RDMSimConnector.timestep);
		// RDMSimConnector.timedRDMSim.network.setTopologyStrategy( RDMSimConnector.timestep);

		


		// double bw = RDMSimConnector.timedRDMSim.network.getBandwidthHistory().get(RDMSimConnector.timestep);
		//  = RDMSimConnector.timedRDMSim.network.getActiveLinksHistory().get(RDMSimConnector.timestep);
		double ttw = RDMSimConnector.timedRDMSim.network.getTtwHistory().get(RDMSimConnector.timestep);

		System.out.println("::::::::Bandwidth Consumption::::"+bw+"   Active Links::::"+al+"  Writing Time:::"+ttw+"TARGET LINKS::::::::::::"+tl+"  Latency:::"+ResultsLog.lat+"---- CURRENT TOPOLOGY::::::"+current+":::::::::::::");
	...
	public int getObservation(int action, int statePrime) {
		...
		// int randomNum = (int)(Math.random() * 27);  // 0 to 100
